...
[ -n "$PLATFORM_NAME" ] && {
    ...
    then
        if [ -z "${APP_NAME}" ]; then
            cmake -S . -B ${BUILD_DIR} -G "Unix Makefiles" -DCMAKE_BUILD_TYPE=Release -DPLATFORM_NAME=${PLATFORM_NAME} -DEXTNL_LIB=N -DSRC_CFG=${SRC_CFG} && {
                cmake --build ${BUILD_DIR} -j 1
            }
        else
            cmake -S . -B ${BUILD_DIR} -G "Unix Makefiles" -DCMAKE_BUILD_TYPE=Release -DPLATFORM_NAME=${PLATFORM_NAME} -DEXTNL_LIB=N -DSRC_CFG=${SRC_CFG} -DAPP_NAME=${APP_NAME} && {
                cmake --build ${BUILD_DIR} -j 1
            }
        fi
        echo
    ...
    then
        ...
        if [ -z "${APP_NAME}" ]; then
            cmake -S . -B ${BUILD_DIR} -G "Unix Makefiles" -DCMAKE_TOOLCHAIN_FILE=${MPFW_CODE_CMAKE_DIR}/arm-none-eabi-gcc.cmake -DCMAKE_BUILD_TYPE=Release -DPLATFORM_NAME=${PLATFORM_NAME} -DEXTNL_LIB=N -DSRC_CFG=${SRC_CFG} && {
                cmake --build ${BUILD_DIR} -j 1
            }
        else
            cmake -S . -B ${BUILD_DIR} -G "Unix Makefiles" -DCMAKE_TOOLCHAIN_FILE=${MPFW_CODE_CMAKE_DIR}/arm-none-eabi-gcc.cmake -DCMAKE_BUILD_TYPE=Release -DPLATFORM_NAME=${PLATFORM_NAME} -DEXTNL_LIB=N -DSRC_CFG=${SRC_CFG} -DAPP_NAME=${APP_NAME} && {
                cmake --build ${BUILD_DIR} -j 1
            }
        fi
